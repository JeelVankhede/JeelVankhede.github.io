...
export function MetricsBar() {
  return (
    <div className="metrics-bar">
      <a href="#top" style={{ fontSize: 13, fontWeight: 700, color: '#b6a8ff', letterSpacing: '-0.02em' }}>
        JV
      </a>
      <div className="metrics-bar-center" style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
        {metrics.map((m, i) => (
          <span key={m.label} style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
            {i > 0 && <span style={{ color: 'rgba(180,168,255,0.15)', padding: '0 10px' }}>|</span>}
      ...
      <div style={{ display: 'flex', gap: 16 }}>
        {[
          ...
          <a
            ...
            style={{ fontSize: 11, color: 'rgba(180,168,255,0.4)', transition: 'color 0.15s' }}
            onMouseEnter={e => (e.currentTarget.style.color = '#b6a8ff')}
            onMouseLeave={e => (e.currentTarget.style.color = 'rgba(180,168,255,0.4)')}
          >
            {l.label}
          </a>
        ))}
